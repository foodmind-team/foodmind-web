...
export function RecipeLibraryPage() {
  ...
  return (
    <div className="page section-page recipe-library-page">
      <header className="section-page-heading"><div><p className="eyebrow">Your cooking shelf</p><h1>Recipes you can actually cook.</h1><p>Recipes are stored in your FoodMind account and are available to Web, Backend, Agent, and future Android clients.</p></div><div className="heading-actions"><Link className="secondary-action" to="/saved/recipes/new"><Plus size={17} /> Add recipe</Link><Link className="primary-action" to="/cooking/import"><Bot size={17} /> Import with Agent</Link></div></header>
      <SavedSectionTabs />
  ...
  return (
    <div className="page section-page recipe-editor-page">
      ...
      <form className="recipe-editor-form" onSubmit={submit} noValidate>
        ...
        <section className="recipe-editor-section"><div className="section-topline"><div><p className="eyebrow">Method</p><h2>How it comes together</h2></div><button className="secondary-action" type="button" disabled={steps.fields.length >= 100} onClick={() => steps.append({ value: '' })}><Plus size={15} /> Add step</button></div><div className="recipe-step-list">{steps.fields.map((field, index) => <label key={field.id}><span>{index + 1}</span><textarea rows={2} maxLength={1_000} placeholder="Write one clear cooking instruction" {...register(`steps.${index}.value`)} /><button className="icon-button" type="button" aria-label={`Remove step ${index + 1}`} disabled={steps.fields.length === 1} onClick={() => steps.remove(index)}><Minus size={16} /></button></label>)}</div></section>
        <div className="recipe-editor-actions"><span><Server size={16} /> Saved to your FoodMind account.</span><Link className="secondary-action" to="/saved/recipes">Cancel</Link><button className="primary-action" type="submit" disabled={save.isPending}><Check size={17} /> {save.isPending ? 'Saving…' : 'Save recipe'}</button></div>
      </form>
    </div>
  )
}
