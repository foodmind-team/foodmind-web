...
import { ArrowLeft, ArrowRight, Check, ListChecks, PackageCheck, ShoppingBasket } from 'lucide-react'
...
export function ShoppingListIndexPage() {
  ...
  return (
    <div className="page section-page shopping-index-page">
      ...
      <section className="shopping-list-grid">{((lists.data?.items || []) as Schema<'ShoppingListResponse'>[]).map((list) => <Link className="shopping-list-card" to={`/shopping-lists/${list.shoppingListId}`} key={list.shoppingListId}><span><ShoppingBasket size={22} /></span><div><p className="eyebrow">{list.status}</p><h2>{list.checkedItemCount} of {list.totalItemCount} purchased</h2><small>Updated {new Date(list.updatedAt).toLocaleString('en-SG')}</small></div><ArrowRight size={18} /></Link>)}</section>
    </div>
  )
}
...
export function ShoppingListDetailPage() {
  ...
  const complete = useMutation({
    ...
    onSuccess: (plan) => {
      ...
      showToast('Shopping list completed.')
  ...
  return (
    <div className="page section-page shopping-detail-page">
      ...
      <header className="section-page-heading"><div><p className="eyebrow">{data.status} · {data.checkedItemCount}/{data.totalItemCount} purchased</p><h1>Shopping list.</h1><p>Check off each item as you shop.</p></div><span className="cooking-mark"><ListChecks /></span></header>
...
function ShoppingItemEditor({ item, list }: {
  ...
  const update = useMutation({
    mutationFn: async (checked: boolean) => dataOrThrow<Schema<'ShoppingListResponse'>>(await api.PATCH('/shopping-lists/{shoppingListId}/items/{itemId}', {
      ...
      body: { checked, purchasedQuantity: item.purchasedQuantity ?? item.requiredQuantity, unit: item.unit, expiryDate: null },
    })),
    onSuccess: (updated) => {
