...
export function ChatConversationPage() {
  ...
  const send = useMutation({
    mutationFn: async (message: string) => dataOrThrow<Schema<'ChatMessageResponse'>>(await api.POST('/chat/sessions/{sessionId}/messages', { body: { content: message }, params: { path: { sessionId } } })),
    onMutate: () => setContent(''),
    onSuccess: async () => {
      await Promise.all([
