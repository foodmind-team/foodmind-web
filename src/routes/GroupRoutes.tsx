...
export function GroupsPage() {
  ...
  return (
    <div className="page section-page">
      ...
      {showCreate && <section className="drawer-card"><div className="section-topline"><div><p className="eyebrow">New trusted space</p><h2>Create a group</h2></div></div>{create.isError && <div className="form-alert" role="alert">{errorMessage(create.error)}</div>}<div className="form-grid"><label>Group name<input value={name} maxLength={120} onChange={(event) => setName(event.target.value)} /></label><label>Description<input value={description} maxLength={500} onChange={(event) => setDescription(event.target.value)} /></label></div><div className="form-actions"><button className="secondary-action" type="button" onClick={() => setShowCreate(false)}>Cancel</button><button className="primary-action" type="button" disabled={!name.trim() || create.isPending} onClick={() => create.mutate()}>{create.isPending ? 'Creating…' : 'Create group'}</button></div></section>}
...
export function JoinGroupPage() {
  ...
  const join = useMutation({
    mutationFn: async () => dataOrThrow<Schema<'GroupMemberResponse'>>(await api.POST('/group-invitations/join', { body: { token } })),
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: queryKeys.groups.list() }); showToast('You joined the group.'); navigate('/groups') },
  })
...
export function GroupWorkspacePage() {
  ...
  const edit = useMutation({
    mutationFn: async () => dataOrThrow<Schema<'GroupResponse'>>(await api.PATCH('/groups/{groupId}', { body: { name, description: description || null }, params: { path: { groupId } } })),
    onSuccess: (updated) => { queryClient.setQueryData(queryKeys.groups.detail(groupId), updated); void queryClient.invalidateQueries({ queryKey: queryKeys.groups.list() }); setEditing(false); showToast('Group details updated.') },
  })
  ...
  const removeMember = useMutation({
    ...
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: queryKeys.groups.members(groupId) }); void queryClient.invalidateQueries({ queryKey: queryKeys.groups.list() }); showToast('Membership updated.') },
  })
  ...
  return (
    <div className="page section-page">
      ...
      <header className="featured-group compact-feature"><div className="featured-group-copy"><span className="group-label"><Users size={15} /> {sentenceCase(group.data?.status)}</span><h1>{group.data?.name}</h1><p>{group.data?.description || 'A private FoodMind decision space.'}</p><div className="header-button-row"><Link className="generate-button small" to={`/?groupId=${groupId}`}><WandSparkles size={18} /> Recommend for this group</Link>{isOwner && <button className="secondary-action inverse" type="button" onClick={() => { setName(group.data!.name); setDescription(group.data!.description || ''); setEditing(true) }}>Edit group</button>}</div></div><div className="group-privacy-card"><p className="eyebrow">Permission safe</p><h2>Shared here, not everywhere.</h2><p>Only active members can read this workspace and its live feed.</p></div></header>
      {editing && <section className="drawer-card"><h2>Edit group</h2><div className="form-grid"><label>Name<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>Description<input value={description} onChange={(event) => setDescription(event.target.value)} /></label></div>{edit.isError && <div className="form-alert">{errorMessage(edit.error)}</div>}<div className="form-actions"><button className="secondary-action" type="button" onClick={() => setEditing(false)}>Cancel</button><button className="primary-action" type="button" onClick={() => edit.mutate()} disabled={!name.trim() || edit.isPending}>Save group</button></div></section>}
      <div className="workspace-grid">
        <section className="workspace-card"><div className="section-topline"><div><p className="eyebrow">Active members</p><h2>{members.data?.length || 0} people</h2></div>{isOwner && <button className="text-button" type="button" onClick={() => invite.mutate()} disabled={invite.isPending}><KeyRound size={16} /> Invite</button>}</div>{members.isError && <ErrorState error={members.error} onRetry={() => void members.refetch()} />}<div className="member-list">{members.data?.map((member) => <div className="member-row" key={member.userId}><span className="member-avatar mint">{(member.displayName || '?').slice(0, 2).toUpperCase()}</span><div><strong>{member.displayName || 'Member'}</strong><small>{sentenceCase(member.role)} · joined {formatDateTime(member.joinedAt)}</small></div>{(isOwner && member.userId !== user?.id) || member.userId === user?.id ? <button className="icon-button" type="button" aria-label={member.userId === user?.id ? 'Leave group' : `Remove ${member.displayName}`} onClick={() => member.userId && removeMember.mutate(member.userId)}><UserMinus size={17} /></button> : null}</div>)}</div>{invitation && <div className="invitation-once" role="status"><strong>Copy this token now—it is shown once.</strong><code>{invitation}</code><button className="secondary-action" type="button" onClick={() => void navigator.clipboard.writeText(invitation).then(() => showToast('Invitation token copied.'))}><Clipboard size={16} /> Copy token</button></div>}{invite.isError && <div className="inline-error">{errorMessage(invite.error)}</div>}</section>
        <section className="workspace-card feed-card"><div className="section-topline"><div><p className="eyebrow">Group activity</p><h2>Trusted feed</h2></div></div>{feed.isLoading && <LoadingState label="Loading group activity…" />}{feed.isError && <ErrorState error={feed.error} onRetry={() => void feed.refetch()} />}{feed.isSuccess && feedItems.length === 0 && <EmptyState title="Nothing shared yet" message="Add a group-visible record or share a recommendation to begin." />}{feedItems.map((item) => <article className="feed-row" key={`${item.sourceType}-${item.sourceId}`}><span className="feed-icon">{item.sourceType === 'RECOMMENDATION_SHARE' ? <Send size={17} /> : <Utensils size={17} />}</span><div><p className="eyebrow">{item.actorDisplayName || 'Group member'} · {formatDateTime(item.occurredAt)}</p><h3>{item.mealNameSnapshot || 'Shared recommendation'}</h3>{item.message && <p>{item.message}</p>}</div></article>)}{feed.hasNextPage && <button className="secondary-action load-more" type="button" onClick={() => void feed.fetchNextPage()} disabled={feed.isFetchingNextPage}>{feed.isFetchingNextPage ? 'Loading…' : 'Load more activity'}</button>}</section>
      </div>
