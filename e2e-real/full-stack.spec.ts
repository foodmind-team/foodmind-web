...
test.describe.serial('real FoodMind stack without route interception', () => {
  ...
  test('performs inventory and cloud recipe CRUD through visible controls', async ({ page }) => {
    ...
    const createLotForm = page.locator('form').filter({
      has: page.getByRole('heading', { name: 'Create a lot' }),
    })
    await createLotForm.getByRole('button', { name: 'Add ingredient', exact: true }).click()
